...
void filterCLI(const char * nombre_col,
    const char * operator,
        const char * value);
...
int encontrarIndiceColumna(Dataframe * df,
...
void verificarNulos(char * linea, int fila, Dataframe * df) {
    ...
    char resultado[MAX_LINE_LENGTH * 2] = {
        0
    }; // Buffer para almacenar la línea procesada
    ...
    strcpy(linea, resultado); // Copiar el resultado de vuelta a la línea original
}